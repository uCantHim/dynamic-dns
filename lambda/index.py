...
def route53_client(execution_mode, route_53_zone_id,
    ...
    if execution_mode == 'set_record':
        try:
            ...
            return [500, {'return_status': 'fail', 'return_message': f'Unknown error: {err}'}]
        except:
...
'''
def run_set_mode(ddns_hostname, validation_hash, source_ip) -> tuple[int, dict]:
    ...
    try:
        ...
        return_message='There was an issue finding '\
            f'or reading {ddns_hostname} configuration from dynamoDB table: ' + \
            str(os.environ.get("ddns_config_table"))
    ...
    if not calculated_hash == validation_hash:
        ...
        return_message='Validation hashes do not match: ' \
            f'{calculated_hash} (expected) vs {validation_hash} (given). ' \
            f'Client IP: {source_ip}, Hostname: {ddns_hostname}'
        return (401, {
...
def lambda_handler(event, context):
    def make_response(status_code: int, status: str, msg: str):
        return {
    ...
    if execution_mode not in execution_modes:
        ...
        return make_response(400, return_status, return_message)

    # For get mode, reflect the client's public IP address and exit.
    if execution_mode == 'get':
        return make_response(200, 'success', source_ip)

    # Proceed with set mode to create or update the DNS record.
    else:
        assert(execution_mode == 'set')
        validation_hash = json.loads(event['body'])['validation_hash']
        ddns_hostname = json.loads(event['body'])['ddns_hostname']

        res = run_set_mode(ddns_hostname, validation_hash, source_ip)
        return make_response(res[0], res[1]['return_status'], res[1]['return_message'])
